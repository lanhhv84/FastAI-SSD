...
def convert_pred(out):
    ...
    return torch.cat(pred, dim=2).cpu().detach().numpy()  # (bs, 4, 8732)
...
def boxes(out, conf, conf_thres=0.7, nms_thres=0.7):
    overlap = iou(out, out)
    for i in range(8732):
        overlap[i, i] = 0
    for i in range(8732):
        for j in range(8732):
            if overlap[i, j] > nms_thres:
                if torch.max(conf[:, i]) > torch.max(conf[:, j]):
                    conf[:, j] = 0
    indices = torch.max(conf, dim=0)[0] > conf_thres
    out = overlap[:, indices]
    conf = conf[:, indices]
    return out, conf
